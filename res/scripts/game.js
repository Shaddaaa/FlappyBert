class FlappyBird{
    ...
    async start(){
    	this.score = 0;
        this.running = true;
        this.tubes = new List([new Tube(this.gap_height, this.cvs), new Tube(this.gap_height, this.cvs, this.cvs.width + this.tube_spacing), new Tube(this.gap_height, this.cvs, this.cvs.width + this.tube_spacing * 2)]);
	...
	update_tubes(){
		...
		for(let i = 0; i < this.tubes.length; i++){
			//move every tube
			this.tubes.get(i).move(this.tube_speed);
			//remove the tubes once they reached the left side, add a new tube to the right
			if(this.tubes.get(i).getX() <= 0 - this.tube_width){
				this.tubes.remove(i);
				this.add_tube();
			}
			//paint top and bottom parts of the tubes
			this.ctx.drawImage(this.tube_imgs["down"], this.tubes.get(i).getX(), this.tubes.get(i).getGap() - this.tube_imgs["down"].height);
			this.ctx.drawImage(this.tube_imgs["up"], this.tubes.get(i).getX(), this.tubes.get(i).getGap() + this.gap_height);
			//collision with Flappy
       		let x_diff = this.player.getX() - this.tubes.get(i).getX();
        	if(x_diff > -1*this.player.getWidth() && x_diff < this.tube_width  && (this.player.getY() <= this.tubes.get(i).getGap() || this.player.getY() + this.player.getHeight() >= this.tubes.get(i).getGap() + this.gap_height)) {
        		this.game_over();
        		break;
        	}
        	//score system
